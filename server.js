...
const recipeUnits=new Set(['mg','g','hg','kg','ml','cl','dl','l','krm','tsk','msk','st','portion','portions']);
function cleanImportedIngredient(item={}){let amount=numberOrNull(item.original_amount??item.amount),unit=String(item.original_unit??item.unit??'').trim().toLowerCase().replace(/\.$/,'');let name=String(item.name??'').trim();const aliases={gram:'g',gramm:'g',grams:'g',milligram:'mg',milligrams:'mg',liter:'l',litre:'l',deciliter:'dl',centiliter:'cl','tsp':'tsk','tbsp':'msk','teaspoon':'tsk','tablespoon':'msk','piece':'st','pieces':'st'};unit=aliases[unit]||unit;if(/^ca\.?$|^cirka$/i.test(unit)){unit='';amount=null;}const embedded=name.match(/^(?:ca\.?|cirka\s+)?(\d+(?:[.,]\d+)?|\d+\s*\/\s*\d+)\s*(mg|g|hg|kg|ml|cl|dl|l|krm|tsk|msk|st|portion(?:er|s)?|tbsp|tsp)\s+(.+)$/i);if(embedded&&(!recipeUnits.has(unit)||amount==null)){amount=numberOrNull(embedded[1].replace(/\s/g,''));unit=(aliases[embedded[2].toLowerCase()]||embedded[2].toLowerCase());name=embedded[3].trim();}if(amount!=null)name=name.replace(/^(?:ca\.?|cirka\s+)?\d+(?:[.,]\d+)?\s+/i,'');if(!recipeUnits.has(unit))unit='unresolved';return {...item,name,amount,unit,original_amount:amount,original_unit:unit==='unresolved'?'':unit,amount_grams:numberOrNull(item.amount_grams)};}
