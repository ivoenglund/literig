...
export async function importFullFineli(pool) {
  ...
  try {
    ...
    for (let offset = 0; offset < values.length; offset += 1000) {
      const batch = values.slice(offset, offset + 1000).filter(v => names.has(v.FOODID) && componentMap.has(v.EUFDNAME) && Number.isFinite(number(v.BESTLOC)));
      const sql = [], args = [];
    ...
    return { skipped: false, foods: foods.length, nutrientValues: values.length };
