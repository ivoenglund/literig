  ...
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE status='verified' AND basis_amount=100 AND basis_unit='g' AND fineli_food_id IS NOT NULL
        AND NOT (state IN ('MIX', 'REC', 'DRINK') OR name ~* '(wok|soup|stew|hamburger|casserole|gravy|dessert|ice cream|frankfurter|noodle|pizza|sandwich|salad|curry|smoothie|juice|drink|beverage|babyfood|filled|cutlet|patty|pastie|powder|with salt|with butter|in milk)')
        AND NOT EXISTS (
...
app.put('/api/recipe-entries/:entryId', async (req, res) => {
  ...
  try {
    ...
    const safeIngredients = await verifiedFineliIngredients(pool, ingredients);
    if (update_standard) {
      await pool.query('DELETE FROM recipe_ingredients WHERE recipe_id=$1', [recipeId]);
      for (const [i, item] of safeIngredients.entries()) await pool.query('INSERT INTO recipe_ingredients (recipe_id, ingredient_name, amount, unit, food_id, sort_order) VALUES ($1,$2,$3,$4,$5,$6)', [recipeId, item.name, Number(item.amount), item.unit || 'g', item.food_id || null, i]);
    }
