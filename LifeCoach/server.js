...
function imageCandidates(html){return [...new Set([...html.matchAll(/<meta[^>]+(?:property|name)=["'](?:og:image|twitter:image)["'][^>]+content=["']([^"']+)/gi)].map(m=>m[1]).filter(x=>/^https?:\/\//i.test(x)))].slice(0,10)}
...
async function callFalRouter(messages){const key=process.env.FAL_KEY||process.env.OPENROUTER_API_KEY;if(!key)throw new Error('FAL_KEY is not configured on lifecoach-api');const response=await fetch('https://fal.run/openrouter/router/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Key ${key}`},body:JSON.stringify({model:'google/gemini-2.5-flash',temperature:0,response_format:{type:'json_object'},messages})});if(!response.ok)throw new Error(`AI gateway returned ${response.status}`);const data=await response.json();return JSON.parse(data.choices?.[0]?.message?.content||'{}')}

app.post('/api/recipe-import/preview', async (req,res)=>{const url=safeRecipeUrl(req.body?.url);if(!url)return res.status(400).json({error:'A public http(s) recipe URL is required'});try{const page=await fetch(url,{redirect:'follow',headers:{'User-Agent':'literig recipe importer/1.0'}});if(!page.ok)throw new Error(`Recipe page returned ${page.status}`);const html=await page.text();const result=await callFalRouter([{role:'system',content:'Extract a recipe for human review. Return JSON only with keys name,description,instructions,ingredients (array of name,amount,unit),image_url. Convert cups, tablespoons, teaspoons and other volume measures to grams using the specific ingredient; all non-null ingredient units must be "g". If a reliable gram conversion is not possible, use amount:null and unit:"g" rather than guessing. Choose one image only from the supplied candidates, or null.'},{role:'user',content:JSON.stringify({url,text:stripHtml(html),image_candidates:imageCandidates(html)})}]);res.json({source_url:url,preview:{name:String(result.name||'Imported recipe'),description:result.description||'',instructions:result.instructions||'',ingredients:Array.isArray(result.ingredients)?result.ingredients:[],image_url:result.image_url||null}})}catch(error){console.error('Recipe import preview failed:',error.message);res.status(502).json({error:'Could not create recipe preview',detail:error.message})}});

app.post('/api/recipe-import/approve', async (req,res)=>{if(!pool)return res.status(503).json({error:'Database not configured'});const p=req.body?.preview;if(!p?.name||!Array.isArray(p.ingredients))return res.status(400).json({error:'A reviewed recipe preview is required'});const client=await pool.connect();try{await client.query('BEGIN');const recipe=(await client.query(`INSERT INTO recipes (name,description,instructions,servings,image_url) VALUES ($1,$2,$3,1,$4) ON CONFLICT(name) DO UPDATE SET description=EXCLUDED.description,instructions=EXCLUDED.instructions,image_url=EXCLUDED.image_url RETURNING id,name,image_url`,[p.name,p.description||'',p.instructions||'',p.image_url||null])).rows[0];await client.query('DELETE FROM recipe_ingredients WHERE recipe_id=$1',[recipe.id]);for(const [i,item] of p.ingredients.entries())await client.query('INSERT INTO recipe_ingredients(recipe_id,ingredient_name,amount,unit,sort_order) VALUES($1,$2,$3,$4,$5)',[recipe.id,String(item.name||'').trim(),item.amount==null?0:Number(item.amount),String(item.unit||'').trim()||'unknown',i]);await client.query('COMMIT');res.status(201).json({recipe})}catch(error){await client.query('ROLLBACK');console.error('Recipe import approval failed:',error.message);res.status(500).json({error:'Could not save imported recipe'})}finally{client.release()}});
...
app.get('/api/recipes', async (_req, res) => {
  if (!pool) return res.status(503).json({ error: 'Database not configured' });
  const result = await pool.query(`SELECT r.id, r.name, r.description, r.instructions, r.servings, r.image_url, COALESCE(json_agg(json_build_object('name',ri.ingredient_name,'amount',ri.amount,'unit',ri.unit,'food_id',ri.food_id,'state',f.state,'source_name',f.source_name,'fineli_food_id',f.fineli_food_id) ORDER BY ri.sort_order) FILTER (WHERE ri.id IS NOT NULL), '[]') AS ingredients FROM recipes r LEFT JOIN recipe_ingredients ri ON ri.recipe_id=r.id LEFT JOIN foods f ON f.id=ri.food_id GROUP BY r.id ORDER BY r.name`);
